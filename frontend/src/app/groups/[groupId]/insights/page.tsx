...
export default function GroupInsightsPage() {
  ...
  let unusualList: { date: string; description: string; amount: number; reason: string; explanation?: string }[] = [];
            ...
            <div className="space-y-2">
              {unusualList.slice(0, 20).map((u, i) => (
                <div key={i} className="rounded-lg bg-amber-500/5 border border-amber-500/10 px-4 py-3">
                  ...
                  {u.explanation && (
                    <p className="mt-2 text-[11px] leading-relaxed text-zinc-500 border-t border-amber-500/10 pt-2">
                      {u.explanation}
                    </p>
                  )}
                </div>
